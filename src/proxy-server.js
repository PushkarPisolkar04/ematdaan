...
// Send access codes endpoint
app.post('/api/send-access-codes', async (req, res) => {
  try {
    const { email, organizationName, accessCodes } = req.body;

    if (!email || !organizationName || !accessCodes) {
      return res.status(400).json({
        success: false,
        message: 'Email, organization name, and access codes are required',
      });
    }
    ...
    res.json({ success: true, message: 'Access codes sent successfully' });
  } catch (error) {
    console.error('Failed to send access codes email:', error);
    res.status(500).json({
      success: false,
